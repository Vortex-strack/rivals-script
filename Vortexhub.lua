...
-- Handles execution across environments
if gethui then
...
local function UpdateESP()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Team ~= LocalPlayer.Team then
            local character = player.Character
            if character then
                if not ESPHighlights[player] or ESPHighlights[player].Parent ~= character then
...
local function HandleNoClip()
    if not VortexConfig.NoClip then return end
    local character = LocalPlayer.Character
    if character then
...
RunService.RenderStepped:Connect(function()
    ...
    FOVCircle.Visible = VortexConfig.Aimbot

    -- Aimbot Logic
    if VortexConfig.Aimbot and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
